...
assert np.min(sv) > 1e-6
print(f"    Singular values: {np.round(sv,6)} -- all nonzero, S_final is invertible.")

# ---------------- the payoff: compare quartic invariants through this verified intertwiner ----------------
lines=[[0,3,4],[1,3,5],[0,1,2],[2,4,5],[2,3,6],[0,5,6],[1,4,6]]
def my_invariants(phi6):
    ...
    Iprod = (sum(phi7[a]*phi7[b]*phi7[c] for (a,b,c) in lines))**2
    tot = sum(x**2 for x in phi7)
    Imixed = sum((phi7[a]+phi7[b]+phi7[c])**2*(tot-phi7[a]**2-phi7[b]**2-phi7[c]**2) for (a,b,c) in lines)
    return [Ig1,Ig2,Iline,Iprod,Imixed]

def their_invariants(chi):
...
for _ in range(200):
    ...
    chi = S_final @ phi6
    rows.append(my_invariants(phi6) + their_invariants(chi))
Amat = np.array(rows)
mine = Amat[:,:5]; theirs = Amat[:,5:]
...
print(f"\n[10] PAYOFF: real-field invariant match (this correspondence's 5 vs King-Luhn's I1,I2,I3,I4,I6):")
print(f"     max fit error = {fit_err:.2e}  <-- machine precision = PERFECT match")
assert fit_err < 1e-10

print("\nDone. Correct basis found and fully verified. See module docstring for the")
print("remaining I5-vs-antisymmetric-invariant open end.")
