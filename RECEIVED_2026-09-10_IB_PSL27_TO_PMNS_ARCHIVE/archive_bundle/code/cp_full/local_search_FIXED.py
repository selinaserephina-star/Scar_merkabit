...
def takagi(Mnu):
    """Correct Takagi decomposition for complex SYMMETRIC matrix (not Hermitian eigh!)."""
    H = Mnu.conj().T @ Mnu
...
def evaluate_point(a2,b3,g1, cl_warm, nu_warm, seed=0, nu_restarts=60):
    best_cl=None
    for p0 in [cl_warm, cl_warm*1.05, cl_warm*0.95]:
    ...
    if best_cl[0] > 1e-15:
        for _ in range(30):
            p0=np.array([rng.uniform(1,20), rng.uniform(-3,3), rng.uniform(0.1,5)])
            res=least_squares(cl_resid,p0,args=(a2,b3,g1),method='lm',max_nfev=3000,xtol=1e-15,ftol=1e-15)
            c=np.sum(res.fun**2)
    ...
    def nu_resid(p):
        ...
        dnu, Wt = takagi(Mnu)     # CORRECT: Takagi, not eigh
        PMNS = UL.conj().T @ Wt
    ...
    best_nu=None
    for p0 in [nu_warm, nu_warm*1.1, nu_warm*0.9]:
    ...
    for _ in range(nu_restarts):
        p0=rng.uniform(-200,200,4)
    ...
    return dict(cl_cost=best_cl[0], nu_cost=best_nu[0], th12=np.degrees(th12),th23=np.degrees(th23),
                ...
                y2=y2,c3=c3,c3p=c3p, m0=m0,m2=m2,m3=m3,mcross=mcross)

if __name__=="__main__":
    cl_warm = np.load("start_cl_params.npy")
    nu_warm = np.array([-3470.63, 67742.36, 5941.76, -441.89])
    a2_0,b3_0,g1_0 = 5.06, 2.43, 1.81
    results={}
    rng = np.random.default_rng(555)
    N = 80
    pts = np.array([a2_0,b3_0,g1_0]) + rng.uniform(-0.5,0.5,size=(N,3))
    for i,(a2,b3,g1) in enumerate(pts):
        r = evaluate_point(a2,b3,g1, cl_warm, nu_warm, seed=i+2000, nu_restarts=60)
        if r is not None:
            results[(round(a2,4),round(b3,4),round(g1,4))] = r
            flag = " <<<< TARGET" if -174<=r['deltaCP']<=-122 else ""
            print(f"[{i}] a2={a2:.3f} b3={b3:.3f} g1={g1:.3f}: cl={r['cl_cost']:.1e} nu={r['nu_cost']:.4f} "
                  f"th12={r['th12']:.1f} th23={r['th23']:.1f} th13={r['th13']:.1f} ratio={r['ratio']:.4f} "
                  f"dCP={r['deltaCP']:.1f} Smnu={r['Summnu']:.3f}{flag}", flush=True)
    with open('local_search_FIXED_results.pkl','wb') as f: pickle.dump(results,f)
