...
def full_residual(p, chi_ax, xi_ax):
    th_phi,y2,c3,c3p,m0,m2,m3,mcross = p
    axc, axx = axis3[chi_ax], axis3[xi_ax]
...
for chi_ax in range(3):
  for xi_ax in range(3):
    if chi_ax == xi_ax:
        results[(chi_ax,xi_ax)] = None
        print(f"chi_ax={chi_ax} xi_ax={xi_ax}: SKIPPED (same-axis, known degenerate)")
        continue
    best=None
    rng = np.random.default_rng(chi_ax*10+xi_ax+1)
    for trial in range(120):
        ...
        try:
            res = least_squares(full_residual, p0, args=(chi_ax,xi_ax), method='trf', max_nfev=3000, xtol=1e-14, ftol=1e-14)
    ...
    print(f"chi_ax={chi_ax} xi_ax={xi_ax}: best cost = {best[0]:.4e}", flush=True)

import pickle
with open('all_axes_free_phi.pkl','wb') as f: pickle.dump(results,f)
