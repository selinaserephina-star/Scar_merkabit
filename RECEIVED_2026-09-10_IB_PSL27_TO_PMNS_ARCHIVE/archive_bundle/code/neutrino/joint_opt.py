...
for trial in range(2000):
    ...
    try:
        ...
        if np.isfinite(cost) and (best is None or cost<best[0]):
            best=(cost,res.x)
            if trial % 50 == 0:
                print(f"trial {trial}: best cost so far = {best[0]:.6e}", flush=True)
    except Exception: continue
    if trial % 100 == 0:
        print(f"...trial {trial} done, best={best[0] if best else None:.6e}" if best else f"...trial {trial}", flush=True)

print("FINAL Best combined cost:", best[0])
print("params:", best[1])
