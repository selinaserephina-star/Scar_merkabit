...
def build_Mnu(m0,m2,m3, chi_axis_idx, xi_axis_idx, ratio):
    chi = axis3[chi_axis_idx]; xi_c3p = ch_fits[(chi_axis_idx,xi_axis_idx)][1][2]
    xi = xi_c3p*axis3[xi_axis_idx]
    cx3 = cross3(chi,xi)
    mcross = ratio * m3
    return m0*np.eye(3) + m2*Y2_of(*phi) + m3*Y3_of(axis3[chi_axis_idx]) + mcross*Y3_of(cx3)

def residual(p, chi_ax, xi_ax, UL, ratio):
    m0,m2,m3 = p
    Mnu = build_Mnu(m0,m2,m3,chi_ax,xi_ax,ratio)
...
print(f"{'chi':>4}{'xi':>4}{'ch.cost':>12}{'nu.cost':>10}{'COMBINED':>12}{'th12':>8}{'th23':>8}{'th13':>8}")
for chi_ax in range(3):
  for xi_ax in range(3):
    ...
    ratio_here = eps * c3p_here
    ch_cost = ch_fits[(chi_ax,xi_ax)][0]
    UL = UL_dict[(chi_ax,xi_ax)]
    best=None
    rng=np.random.default_rng(chi_ax*10+xi_ax+5)
    for _ in range(400):
        p0 = rng.uniform(-15,15,3)
        try:
            res=least_squares(residual,p0,args=(chi_ax,xi_ax,UL,ratio_here),method='lm',max_nfev=5000,xtol=1e-15,ftol=1e-15)
            cost=np.sum(res.fun**2)
            if np.isfinite(cost) and (best is None or cost<best[0]): best=(cost,res.x)
        except Exception: continue
    nu_cost,p=best
    m0,m2,m3=p
    Mnu=build_Mnu(m0,m2,m3,chi_ax,xi_ax,ratio_here)
    w,V=np.linalg.eigh(Mnu)
    order=np.argsort(np.abs(w)); w=w[order]; V=V[:,order]; masses=np.abs(w)
    PMNS=UL.T@V
    th12,th23,th13=pmns_angles(PMNS)
    print(f"{chi_ax:>4}{xi_ax:>4}{ch_cost:>12.2e}{nu_cost:>10.4f}{ch_cost+nu_cost:>12.4e}{np.degrees(th12):>8.2f}{np.degrees(th23):>8.2f}{np.degrees(th13):>8.2f}")
