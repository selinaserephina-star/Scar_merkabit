...
def build_Mnu(m0,m2,m3,mcross,m5, chi_ax, cx3_dir, ab2_dir):
    return (m0*np.eye(3) + m2*Y2_of(*phi) + m3*Y3_of(axis3[chi_ax]) + mcross*Y3_of(cx3_dir)
            + m5*Y2_of(*ab2_dir))

def eval_point(p, chi_ax, kappa, cx3_dir, ab2_dir, UL):
    m0,m2,m3,m5 = p
    mcross = kappa*m3   # kappa already includes eps*c3p_here factor baked in by caller
    Mnu = build_Mnu(m0,m2,m3,mcross,m5,chi_ax,cx3_dir,ab2_dir)
...
def residual4(p, chi_ax, kappa, cx3_dir, ab2_dir, UL):
    r = eval_point(p,chi_ax,kappa,cx3_dir,ab2_dir,UL)
...
def best_fit(chi_ax, xi_ax, kappa_raw, nstarts=200, seed=0):
    ...
    ab2 = channel2_from_squares(chi_v)
    if np.linalg.norm(ab2)<1e-12: return None
    ab2_dir = ab2/np.linalg.norm(ab2)
    kappa_eff = kappa_raw*eps*c3p_here   # kappa_raw is the O(1) dimensionless number scanned
    ...
    for _ in range(nstarts):
        ...
        try:
            res=least_squares(residual4,p0,args=(chi_ax,kappa_eff,cx3_dir,ab2_dir,UL),
                               method='lm',max_nfev=3000,xtol=1e-14,ftol=1e-14)
            c=np.sum(res.fun**2)
            if np.isfinite(c) and (best is None or c<best[0]): best=(c,res.x)
        except Exception: continue
    if best is None: return None
    r = eval_point(best[1],chi_ax,kappa_eff,cx3_dir,ab2_dir,UL)
    return best[0], best[1], r

print(f"{'kappa':>7}{'cost(2,0)':>12}{'th12':>8}{'th23':>8}{'th13':>8}{'Sum_mnu':>10}{'best_other':>12}{'ratio':>10}")
results = {}
kappas = np.arange(0.5, 2.01, 0.05)
for kappa in kappas:
    res_20 = best_fit(2,0,kappa,nstarts=150,seed=int(kappa*1000))
    if res_20 is None:
        print(f"{kappa:>7.2f}  FAILED")
        continue
    c20, p20, r20 = res_20
    # check other axes at SAME kappa for discrimination
    other_costs = []
    for chi_ax in range(3):
        for xi_ax in range(3):
            if chi_ax==xi_ax or (chi_ax,xi_ax)==(2,0): continue
            res_o = best_fit(chi_ax,xi_ax,kappa,nstarts=60,seed=int(kappa*1000)+chi_ax*10+xi_ax)
            if res_o is not None:
                other_costs.append(res_o[0])
    best_other = min(other_costs) if other_costs else float('nan')
    results[kappa] = (c20,p20,r20,best_other)
    ratio_disc = best_other/c20 if c20>1e-12 else float('inf')
    print(f"{kappa:>7.2f}{c20:>12.5f}{np.degrees(r20[0]):>8.2f}{np.degrees(r20[1]):>8.2f}{np.degrees(r20[2]):>8.2f}{r20[4]:>10.4f}{best_other:>12.5f}{ratio_disc:>10.2f}")

with open('kappa_m5_results.pkl','wb') as f: pickle.dump(results,f)
