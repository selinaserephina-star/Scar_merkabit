...
def residual(p, chi_ax, xi_ax, UL, c3p):
    ...
    k = np.sqrt(Dm31_phys/dm31) if dm31>1e-12 else 1e6
    summnu = np.sum(masses*k)
    return np.array([(th12-th12_t)/th12_t,(th23-th23_t)/th23_t,(th13-th13_t)/th13_t,
                      (rat-ratio_t)/ratio_t, (summnu-Summnu_target)/Summnu_target])

results={}
for (chi_ax,xi_ax) in [(1,0),(2,0)]:
    c3p_here = ch_fits[(chi_ax,xi_ax)][1][2]
    UL = UL_dict[(chi_ax,xi_ax)]
    best=None
    rng=np.random.default_rng(chi_ax*1000+xi_ax+123)
    for _ in range(3000):
        p0 = rng.uniform(-40,40,4)
        try:
            res=least_squares(residual,p0,args=(chi_ax,xi_ax,UL,c3p_here),method='lm',max_nfev=4000,xtol=1e-15,ftol=1e-15)
            cost=np.sum(res.fun**2)
            if np.isfinite(cost) and (best is None or cost<best[0]): best=(cost,res.x)
        except Exception: continue
    results[(chi_ax,xi_ax)]=best
    print(f"chi={chi_ax} xi={xi_ax}: best cost = {best[0]:.6f}", flush=True)

with open('refine_5target_result.pkl','wb') as f: pickle.dump(results,f)
