...
def cl_fit_roleswap(t, nstarts=150, seed=0):
    tau=1j*t
    ...
    def resid(p):
        ...
        Phi3 = n3*np.array([Y3.real,Y4.real,Y5.real])
        Phi3p = n3p*np.array([(Y1*Y4-Y2*Y5).real,(Y1*Y5-Y2*Y3).real,(Y1*Y3-Y2*Y4).real])
...
print(f"{'t':>8}{'cost':>14}")
for t in np.arange(0.85, 4.5, 0.2):
    b = cl_fit_roleswap(t, seed=int(t*1000)+7)
