...
def build_Mnu_seesaw(yD1, yD2, MN, chi_ax, cx3_dir):
    # Genuine type-I seesaw: Y_D = yD1*Y3(Phi3) + yD2*Y3(cross), M_N = MN*I (S4-singlet heavy mass)
    YD = yD1*Y3_of(axis3[chi_ax]) + yD2*Y3_of(cx3_dir)
    Mnu = -(YD @ YD.T) / MN   # heavy mass integrated out: M_nu = -Y_D M_N^-1 Y_D^T, M_N=MN*I
    return Mnu

def eval_point(p, chi_ax, cx3_dir, UL):
    yD1,yD2,MN,m0,m2 = p
    if MN <= 0: return None
    Mnu = build_Mnu_seesaw(yD1,yD2,MN,chi_ax,cx3_dir) + m0*np.eye(3) + m2*Y2_of(*phi)
...
def best_fit(chi_ax, xi_ax, target_mnu, nstarts=300, seed=0):
    ...
    for _ in range(nstarts):
        p0 = np.array([rng.uniform(-5,5), rng.uniform(-5,5), 10**rng.uniform(-2,3),
...
print("=== Genuine type-I seesaw test, target Sum(mnu)=0.06 eV ===\n")
...
for chi_ax in range(3):
  for xi_ax in range(3):
    ...
    r = best_fit(chi_ax,xi_ax,0.06,nstarts=250,seed=chi_ax*10+xi_ax)
    if r is None:
        print(f"{chi_ax:>4}{xi_ax:>4}   FAILED")
        continue
    cost,p,obs = r
...
with open('seesaw_results.pkl','wb') as f: pickle.dump(results,f)
