...
def build_Mnu_radiative(yD1, yD2, M0, M1, loopfac, chi_ax, cx3_dir):
    # Non-universal heavy Majorana mass: M_N = M0*I + M1*Y3(Phi3)  (genuinely NEW ingredient)
    YD = yD1*Y3_of(axis3[chi_ax]) + yD2*Y3_of(cx3_dir)
    MN = M0*np.eye(3) + M1*Y3_of(axis3[chi_ax])
    try:
        MNinv = np.linalg.inv(MN)
    except np.linalg.LinAlgError:
        return None
    Mnu = -loopfac*(YD @ MNinv @ YD.T)
    return Mnu

def eval_point(p, chi_ax, cx3_dir, UL):
    yD1,yD2,M0,M1,loopfac = p
    if abs(M0) < 1e-6: return None
    Mnu = build_Mnu_radiative(yD1,yD2,M0,M1,loopfac,chi_ax,cx3_dir)
    if Mnu is None or not np.all(np.isfinite(Mnu)): return None
...
def best_fit(chi_ax, xi_ax, target_mnu, nstarts=250, seed=0):
    ...
    for _ in range(nstarts):
        p0 = np.array([rng.uniform(-5,5), rng.uniform(-5,5), rng.uniform(0.5,5),
...
print("=== Non-universal heavy mass (M_N = M0*I + M1*Y3(Phi3)), target Sum(mnu)=0.06 eV ===\n")
...
for chi_ax in range(3):
  for xi_ax in range(3):
    ...
    r = best_fit(chi_ax,xi_ax,0.06,nstarts=250,seed=chi_ax*10+xi_ax+200)
    if r is None:
...
with open('radiative_results.pkl','wb') as f: pickle.dump(results,f)
