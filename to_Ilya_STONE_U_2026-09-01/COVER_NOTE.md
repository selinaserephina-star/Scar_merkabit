...
the send is Selina's word, recorded in the registry when made).** Answers your
THREE ANSWERS batch same-day. Two runs: Stone U (brief sha-locked BEFORE code
— lock included; 59/59) and the verification of your normalizer mechanism
(19/19). Package: this cover + brief & lock + both verifiers with logs + the
Stone U findings doc + the kept fail-first log + registry snapshot v0.31 +
SHA256SUMS.txt. (Stone U's checkpoint cache is excluded; the verifier
regenerates it on first run.)
